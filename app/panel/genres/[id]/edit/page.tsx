...
import { getGenreById, updateGenre } from '@/app/actions/genre-actions';

export default function EditGenrePage() {
    const router = useRouter();
    const params = useParams();

    // Pobieramy ID z URL i konwertujemy na number (bo Genre Id to int)
    const genreId = Number(Array.isArray(params.id) ? params.id[0] : params.id);

    const [loading, setLoading] = useState(true);
    const [saving, setSaving] = useState(false);

    // Stan formularza - tylko nazwa
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Wysyłamy obiekt zgodny z UpdateGenreDto { name: string }
        const result = await updateGenre(genreId, { name });
            ...
            <div className="flex-1 max-w-[600px] w-full mx-auto p-4 md:p-8 flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-gray-400 text-sm">
                        Zmień nazwę kategorii filmowej.
                    </p>
                </div>
